...
/**
 * Обработчик выбора офлайн-формата встречи.
 *
 * @param messageService сервис отправки сообщений
 */
@Component
class ReadyOfflineHandler(
    private val messageService: MessageService,
) : Handler {

    override fun accept(command: String, user: User?): Boolean {
        return user != null && user.status == Status.CHOOSING_FORMAT && command == Command.READY_OFFLINE.command
    }
    ...
    override fun handle(update: Update, user: User?) {
        ...
        user.meetingFormat = MeetingFormat.OFFLINE
