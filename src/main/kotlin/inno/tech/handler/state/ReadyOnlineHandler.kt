...
/**
 * Обработчик выбора онлайн-формата встречи.
 *
 * @param messageService сервис отправки сообщений
 */
@Component
class ReadyOnlineHandler(
    private val messageService: MessageService,
) : Handler {

    override fun accept(command: String, user: User?): Boolean {
        return user != null && user.status == Status.CHOOSING_FORMAT && command == Command.READY_ONLINE.command
    }
